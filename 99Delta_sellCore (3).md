...
```cpp
 ...
 Vector3 WorldToScreenX(const Vector3& world, const FMinimalViewInfo& cam)
{

	Vector3 CamLoc =  { cam.Location.x , cam.Location.y, cam.Location.z };
	   
	// 2) world - camera
	Vector3 delta = world - CamLoc;

	// 3) build rotation matrix
	XMMATRIX rotMat = RotatorToMatrix(cam.Rotation);

	// 4) extract axis
	Vector3 axisX(rotMat.r[0].m128_f32[0], rotMat.r[0].m128_f32[1], rotMat.r[0].m128_f32[2]);
	Vector3 axisY(rotMat.r[1].m128_f32[0], rotMat.r[1].m128_f32[1], rotMat.r[1].m128_f32[2]);
	Vector3 axisZ(rotMat.r[2].m128_f32[0], rotMat.r[2].m128_f32[1], rotMat.r[2].m128_f32[2]);
	...
	// --------------------------------------------------------
	// AUTO-FIX 1: ถ้า z ติดลบ แสดงว่าแกนกลับด้าน → กลับแกนให้เอง
	// --------------------------------------------------------
	if (transformed.z < 0.01f)
	{
		// ลองกลับแกน Z เฉย ๆ
		Vector3 transformedFix(
			delta.Dot(axisY),
			delta.Dot(axisZ),
			-delta.Dot(axisX)
		);

		if (transformedFix.z > 0.01f)
			transformed = transformedFix;
	}

	// --------------------------------------------------------
	// AUTO-FIX 2: ถ้ายังติดลบ → ใช้วิธี fallback 
	// --------------------------------------------------------
	if (transformed.z < 0.01f)
	{
		// สุดท้ายใช้ fix แบบ force
		transformed.z = 0.01f;
	}
	...
	// 6) World → Screen
	// --------------------------------------------------------
	float screenX = Width * 0.5f;
	float screenY = Height * 0.5f;

	// UE FOV is horizontal → convert เล็กน้อย
	float fovTan = tanf(cam.FOV * M_PI / 360.f);

	Vector3 out;
	out.x = screenX + transformed.x * (screenX / fovTan) / transformed.z;
	out.y = screenY - transformed.y * (screenX / fovTan) / transformed.z;
	out.z = transformed.z;
...
uintptr_t AcknowledgedPawnTeamChack;
void cache()
{
/*
		//PlayerArray = 0x98
		...
		if (AcknowledgedPawn == 0) AcknowledgedPawn = TrimTrailingNibble(driver.read <uintptr_t>(Base_PlayerController + 0x3E8));//0x400 0x408  // Loacpalyer


      	if (AcknowledgedPawnTeamChack == 0) AcknowledgedPawnTeamChack = TrimTrailingNibble(driver.read <uintptr_t>(Base_PlayerController + 0x3F0));//0x400 0x408
	 	for (int i = 0; i < PlayerCount; ++i)
	      {	 
			auto actor = driver.read<uintptr_t>(PlayerArray + i * 0x8) ;
			 
			 // fprintfEx(console.stream, _("actor > 0x%llX\n"), actor); 
			 if(isBadPtr(actor)) { continue; }

		int32_t FName = driver.read<int32_t>(actor + 0x1C);
std::string validName = GetNameFromFName22233(FName);
				 	  if (validName.find(_("Character_")) != std::string::npos
			  || validName.find(_("DFMCharacter_C")) != std::string::npos 
			 || validName.find(_("LocalPlayer")) != std::string::npos  
			  || validName.find(_("DFMAICharacter")) != std::string::npos
			  || validName.find(_("Inventory_CarryBody_C")) != std::string::npos 
			  || validName.find(_("InventoryPickup")) != std::string::npos  

			  ) 
		 {

			  TslEntity tslEntity{};
			//  tslEntity.MeshPointer = Mesh;

			  if (std::none_of(tmpList.begin(), tmpList.end(), 
				  [actor](const TslEntity& e) 
				  { 
				  return e.pObjPointer == actor; 
				  })) 
			  {
...
/*
	auto RootComponent = TrimTrailingNibble( driver.read<uintptr_t>(entity.pObjPointer + 0x180));//0x188
	if(!RootComponent)RootComponent = TrimTrailingNibble(driver.read<uintptr_t>(entity.pObjPointer + 0x190));

		auto loaclRootComponent = TrimTrailingNibble(driver.read<uintptr_t>(AcknowledgedPawn + 0x190));
	if(!loaclRootComponent)loaclRootComponent = TrimTrailingNibble(driver.read<uintptr_t>(AcknowledgedPawn + 0x180));
		 Vector3 RelativeLocation = driver.read<Vector3>(RootComponent + 0x168);
 Vector3 loaclRelativeLocation = driver.read<Vector3>(loaclRootComponent + 0x88);
  DistanceEnlityAll = GetDistanceM(loaclRelativeLocation, RelativeLocation);
Vector3 X = WorldToScreenX(RelativeLocation, CameraCachePrivate.POV); 
					auto TeamComponent = driver.read<uintptr_t>(entity.pObjPointer + 0x1068); // Type >0x24 VaFname >0x2836E1-> Offset->0xEC8 FName: TeamComponent APtr: 0x2147BFCA0 Adr 0x 1F8E04208-> [Count->1|minCount->3784|maxCount->0x3784 ]


				//auto loaclTeamComponent = driver.read<uintptr_t>(AcknowledgedPawn + 0x1068);
				auto loaclTeamComponent = driver.read<uintptr_t>(AcknowledgedPawnTeamChack + 0x1068); //เช็คทีม 
				// Type >0x2C VaFname >0x90E2B8-> Offset->0x1570 FName: DFMCharacterDataConfig APtr: 0xB8FEAC00 Adr 0x 2091D48B0->
				//		 Type >0x2C VaFname >0x10BA65-> Offset->0x38 FName: GPCharacterDataAsset APtr: 0x183FCE00 Adr 0x B8FEAC38->
				auto TeamId = driver.read<int32_t>(TeamComponent + Team_CampId_Debug); //	int32_t TeamId; // 0x644(0x4) 
				auto loaclTeamId = driver.read<int32_t>(loaclTeamComponent + Team_CampId_Debug); //	int32_t TeamId; // 0x644(0x4)
				auto CampId = driver.read<int32_t>(TeamComponent + (Team_CampId_Debug + 4)); //0x114 int32_t TeamId; // 0x644(0x4) 
				auto loaclCampId = driver.read<int32_t>(loaclTeamComponent + (Team_CampId_Debug + 4)); //	int32_t TeamId; // 0x644(0x4)
				if (visuals::ShowTeamAll == false) {

					if (visuals::Team == true)
					{
						if (loaclCampId == CampId)continue;

					}
					else
					{
						if (loaclTeamId == TeamId)continue;
					}
				}

	auto HealthComp = driver.read<uintptr_t>(entity.pObjPointer + 0x1060);//	class GPHealthDataComponent HealthComp; // 0xE70(0x8)
 
	auto AttributeSetHealth = driver.read<uintptr_t>(HealthComp + 0x248); //	class GPAttributeSetHealth HealthSet; // 0x240(0x8)
	auto HealthEx2 = driver.read<float>(AttributeSetHealth + 0x50);  //100.f
	 
	  if (visuals::Health == true) {
		 if (HealthEx2 <= 0.0000f || HealthEx2 >= 999.0f)
			 continue;
		  
	 }
 

ใน UC มี Item  InventoryPickup นะ  ตกตามพื้น
	   if (validName.find(_("Inventory_CarryBody_C")) != std::string::npos)
{... }else if (validName.find(_("InventoryPickup")) != std::string::npos)
			{...}	else if (validName.find(_("SafeBox")) != std::string::npos)
	{	.... }
...
void menu()
{
	ImGui::BeginChild((xorstr_("##Players22")), ImVec2(-1, 200.f), true);
	{
		ImGui::PushItemWidth(255 );
		ImGui::SliderInt("Offset##Team", &Team_CampId_Debug, 0, 0x800, "0x%X");
		ImGui::PopItemWidth();

		ImGui::SameLine();
		ImGui::SetNextItemWidth(90);
		ImGui::InputInt("##OffsetInput", &Team_CampId_Debug, 1, 16);

		ImGui::PushItemWidth(170.f);
		ImGui::SliderFloat((xorstr_("##FontSize")), &visuals::boxsize, 1.f, 5.f);
		ImGui::PopItemWidth();
...
```
<img width="1238" height="585" alt="image" src="https://github.com/user-attachments/assets/b02495e5-a3cb-4c34-a3d3-6a0ecbbb2d87" />
...
//รวม header อย่างถูกต้อง
//Kernel project:
//---
//#include <ntifs.h>      // Kernel API
...
bool Bone_ValidFloat(float f)
{
	return std::isfinite(f) && fabsf(f) < 100000.f;
}

bool Bone_ValidVector3(const Vector3& v)
{
	return Bone_ValidFloat(v.x) && Bone_ValidFloat(v.y);
}

bool ValidBoneT(const FTransform& t)
{
	return Bone_ValidVector3(t.translation) && Bone_ValidFloat(t.scale.x);
}

bool ValidC2W(const FTransform& t)
{
	if (!Bone_ValidVector3(t.translation)) return false;
	if (fabsf(t.scale.x) < 0.01f || fabsf(t.scale.x) > 4.0f) return false;
	return true;
}

 inline bool IsNibbleTail(uint64_t v)
{
    uint8_t last = v & 0xFF;
    uint8_t hi = (last & 0xF0) >> 4;
    uint8_t lo = (last & 0x0F);
    return (hi == 0 && lo > 0);
}
